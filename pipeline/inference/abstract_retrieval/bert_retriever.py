...
--------------------------------------------------------------
FILE:
    pipeline/inference/abstract_retrieval/bert_retriever.py

NOTE:
    Prior to running this script you must have either trained
    the BERT model through:
...
INFO:
    The script implements the BERTRetriever class that performs
    Abstract Retrieval in "BERT" (i.e. HYBRID) setting. It first
    uses the BM25 algorithm to find 20 matching docs, that are
    further classified by BERT classifier to determine final picks.
...
class BERTRetriever(BioBERTRetriever):
    '''
    -------------------------
    BERTRetriever - Class for Abstract Retrieval in "BERT" (HYBRID) setting.
                       It utilizes a fine-tuned BERT on SciFact.
                       Parent class: BioBERTRetriever.
    -------------------------
    '''

    def __init__(self: 'BERTRetriever') -> None:
        '''
        Initialize the class for AR in "BERT" setting.
        '''
        super().__init__()
        # Over-write the retrieval strategy.
        self.retrieval_strategy = 'BERT'

    def run(self: 'BERTRetriever') -> None:
        '''
        Run the "BERT" retrieval by calling the classifier function
...
# Run the training for AR with BERT.
